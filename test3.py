...
for track_idxs, report_idxs in clusters:
    ...
    for hypothesis, probability in zip(hypotheses, probabilities):
        hypothesis_representation = [(track_idxs[track_idx], report_idxs[report_idx]) if report_idx is not None else (track_idxs[track_idx], None) for track_idx, report_idx in enumerate(hypothesis)]
